...
interface QuickCreateWarehouseModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreated: (warehouse: any) => void;
}

const QuickCreateWarehouseModal: React.FC<QuickCreateWarehouseModalProps> = ({ 
  isOpen, 
  onClose, 
  onCreated 
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', location: '', contactPerson: '', contactNumber: '' });
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl max-w-md w-full border border-gray-200 dark:border-gray-800" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Create Warehouse</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">Add a new warehouse quickly</p>
            </div>
          </div>
          <button
            onClick={handleClose}
...
export default QuickCreateWarehouseModal;
